...
function DeveloperSection() {
  ...
  const handleFile = async (file: File) => {
    if (!file.name.endsWith('.zip')) {
      setResult({ ok: false, message: 'File must be a .zip archive' });
      return;
    }
    setUploading(true);
    setResult(null);
    try {
      // Dev app upload is no longer supported — apps are hosted MCP servers
      setResult({ ok: false, message: 'Dev upload is no longer supported. Apps are now hosted MCP servers at *.apps.construct.computer' });
    } catch (err) {
        ...
        <div className="mt-4">
          <SettingsCard>
            <div className="px-4 py-3 border-b border-black/[0.06] dark:border-white/[0.06]">
              <div className="flex items-center gap-2 mb-1">
                <FileArchive className="w-4 h-4 opacity-50" />
                <span className="text-[13px] font-medium">Install App from ZIP</span>
              </div>
              <p className="text-[11px] text-[var(--color-text-muted)] leading-snug">
                Upload a .zip file containing a Construct app (must include a valid manifest.json).
                If the app is already installed, it will be overwritten.
              </p>
            </div>
            ...
            <div className="p-4">
              ...
              <div
                ...
                onDrop={handleDrop}
                className={`relative flex flex-col items-center justify-center gap-2 py-8 px-4 rounded-lg border-2 border-dashed transition-colors cursor-pointer ${
                  dragOver
                    ? 'border-[var(--color-accent)] bg-[var(--color-accent)]/5'
                    : 'border-black/10 dark:border-white/10 hover:border-black/20 dark:hover:border-white/20'
                } ${uploading ? 'pointer-events-none opacity-50' : ''}`}
                onClick={() => document.getElementById('dev-zip-input')?.click()}
              >
                {uploading ? (
                ...
                <p className="text-xs opacity-40 text-center">
                  {uploading ? 'Installing...' : 'Drag & drop a .zip file here, or click to browse'}
                </p>
                <input
                  id="dev-zip-input"
                  type="file"
              ...
              {result && (
                <div className={`flex items-start gap-2 mt-3 px-3 py-2 rounded-lg text-xs ${
                  result.ok
                    ? 'bg-emerald-500/5 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300'
                    : 'bg-red-500/5 dark:bg-red-500/10 text-red-700 dark:text-red-300'
                }`}>
                  {result.ok
          ...
          <div className="mt-3 px-1">
            <p className="text-[11px] text-[var(--color-text-muted)] leading-relaxed">
              The ZIP should contain a Construct app directory with at least: <code className="px-1 py-0.5 rounded bg-black/[0.04] dark:bg-white/[0.06] font-mono text-[10px]">manifest.json</code> and a server entry point.
              See the <a href="https://github.com/anthropics/construct-app-hello-world" target="_blank" rel="noopener noreferrer" className="text-[var(--color-accent)] hover:underline">hello-world app</a> for reference.
            </p>
          </div>
        </div>
      )}
